    ...
    private fun navigateToMainActivity(user: FirebaseUser?) {
        user?.let {
            ...
            lifecycleScope.launch {
                try {
                    val result = databaseManager.createOrUpdateUser(it)
                    if (result.isSuccess) {
                        Log.d(TAG, "Usuario guardado en Database")
                    } else {
                        Log.e(TAG, "Error al guardar usuario", result.exceptionOrNull())
                    }
                } catch (e: Exception) {
                    Log.e(TAG, "Error en operación de database", e)
                }
            }
        }
